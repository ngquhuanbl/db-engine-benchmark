...
// Create the native browser window.
function createMsgChannelWindow() {
  const mainWindow = new BrowserWindow({
    width: 800,
    height: 600,
    // Set the path of an additional "preload" script that can be used to
    // communicate between node-land and browser-land.
    webPreferences: {
      preload: path.join(__dirname, "message-channel.preload.js"),
      contextIsolation: false,
      nodeIntegration: true,
      sandbox: false,
    },
  ...
  const appURL = app.isPackaged
    ? url.format({
        pathname: path.join(__dirname, "message-channel.html"),
        protocol: "file:",
        slashes: true,
      })
    : "http://localhost:3001";
...
app.whenReady().then(async () => {
  try {
    ...
    msgChannelWindow.once("ready-to-show", () => {
      msgChannelWindow.webContents.postMessage("port", null, [port2]);
    });
