...
export async function execute(
  ...
  async function resetData() {
    const logId = addLog("[sqlite + contextBridge] reset data");
  ...
  // WRITE
  {
    const logId = addLog("[sqlite + contextBridge][n-transaction] write");
  ...
  // READ
  {
    const logId = addLog("[sqlite + contextBridge][n-transaction] read");
    const requests = data.map((jsData: any) => {
      const params: any[] = [];
  ...
  // WRITE
  {
    const logId = addLog("[sqlite + contextBridge][one-transaction] write");
    const start = performance.now();
    let end = -1;
  ...
  // READ
  {
    const logId = addLog("[sqlite + contextBridge][one-transaction] read");
    const start = performance.now();
    let end = -1;
    await new Promise<void>((resolve, reject) => {
      conn.serialize((conn) => {
