...
const originalExecute = async (
  ...
  // DELETE
  {
    const logId = addLog("[msg-channel-sqlite][delete][n-transaction] write");
            ...
            openSQLiteDatabase(partitionKey).then((conn) =>
              conn.get(query, [], (error, row) => {
                if (error) {
                  reject(
                    patchJSError(error, {
                      tags: ["msg-channel-sqlite", "n-transaction", "verify"],
                    })
                  );
                } else {
                  resolve(row["COUNT(*)"]);
  ...
  //   {
  //     const logId = addLog(
  //       "[msg-channel-sqlite][delete][one-transaction] write"
  //     );
