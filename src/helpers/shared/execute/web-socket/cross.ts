...
import * as WebsocketDBExecutor from "../../../renderer/web-socket/actions";
import {
  DEFAULT_LIMIT,
...
export const execute = async (
  ...
  //#region Single read write
  {
    const {
      ...
      oneTransactionRead,
      oneTransactionWrite,
    } = await WebsocketDBExecutor.singleReadWrite(
      benchmarkCount,
      datasetSize,
      DEFAULT_READ_USING_BATCH,
      DEFAULT_READ_BATCH_SIZE,
  ...
  //#region Read by range
  {
    const { nTransactionSum, oneTransactionSum } =
      await WebsocketDBExecutor.readByRange(
        benchmarkCount,
        DEFAULT_READ_USING_BATCH,
        DEFAULT_READ_BATCH_SIZE,
        addConsoleLog,
  ...
  {
    const { nTransactionSum, oneTransactionSum } =
      await WebsocketDBExecutor.readAll(
        benchmarkCount,
        datasetSize,
        DEFAULT_READ_USING_BATCH,
        DEFAULT_READ_BATCH_SIZE,
  ...
  {
    const { nTransactionSum, oneTransactionSum } =
      await WebsocketDBExecutor.readFromEndSource(
        benchmarkCount,
        datasetSize,
        DEFAULT_READ_USING_BATCH,
        DEFAULT_READ_BATCH_SIZE,
  ...
  {
    const { nTransactionSum, oneTransactionSum } =
      await WebsocketDBExecutor.readByIndex(
        benchmarkCount,
        DEFAULT_READ_USING_BATCH,
        DEFAULT_READ_BATCH_SIZE,
        addConsoleLog,
  ...
  {
    const { nTransactionSum, oneTransactionSum } =
      await WebsocketDBExecutor.readByLimit(
        benchmarkCount,
        DEFAULT_READ_USING_BATCH,
        DEFAULT_READ_BATCH_SIZE,
        addConsoleLog,
  ...
  {
    const { nTransactionSum, oneTransactionSum } =
      await WebsocketDBExecutor.readByNonIndex(
        benchmarkCount,
        DEFAULT_READ_USING_BATCH,
        DEFAULT_READ_BATCH_SIZE,
        addConsoleLog,
