...
import * as MsgChannelDBExecutor from "../../../renderer/msg-channel/actions";
import {
  DEFAULT_LIMIT,
...
export const execute = async (
  ...
  //#region Single read write
  {
    const {
      ...
      oneTransactionRead,
      oneTransactionWrite,
    } = await MsgChannelDBExecutor.singleReadWrite(
      benchmarkCount,
      datasetSize,
      DEFAULT_READ_USING_BATCH,
      DEFAULT_READ_BATCH_SIZE,
  ...
  //#region Read by range
  {
    const { nTransactionSum, oneTransactionSum } =
      await MsgChannelDBExecutor.readByRange(
        benchmarkCount,
        DEFAULT_READ_USING_BATCH,
        DEFAULT_READ_BATCH_SIZE,
        addConsoleLog,
  ...
  {
    const { nTransactionSum, oneTransactionSum } =
      await MsgChannelDBExecutor.readAll(
        benchmarkCount,
        datasetSize,
        DEFAULT_READ_USING_BATCH,
        DEFAULT_READ_BATCH_SIZE,
  ...
  {
    const { nTransactionSum, oneTransactionSum } =
      await MsgChannelDBExecutor.readFromEndSource(
        benchmarkCount,
        datasetSize,
        DEFAULT_READ_USING_BATCH,
        DEFAULT_READ_BATCH_SIZE,
  ...
  {
    const { nTransactionSum, oneTransactionSum } =
      await MsgChannelDBExecutor.readByIndex(
        benchmarkCount,
        DEFAULT_READ_USING_BATCH,
        DEFAULT_READ_BATCH_SIZE,
        addConsoleLog,
  ...
  {
    const { nTransactionSum, oneTransactionSum } =
      await MsgChannelDBExecutor.readByLimit(
        benchmarkCount,
        DEFAULT_READ_USING_BATCH,
        DEFAULT_READ_BATCH_SIZE,
        addConsoleLog,
  ...
  {
    const { nTransactionSum, oneTransactionSum } =
      await MsgChannelDBExecutor.readByNonIndex(
        benchmarkCount,
        DEFAULT_READ_USING_BATCH,
        DEFAULT_READ_BATCH_SIZE,
        addConsoleLog,
